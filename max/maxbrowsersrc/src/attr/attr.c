...
/* Laenge des benoetigten Strings berechnen */
  ...
  for (i = 0; i < na; i++)
  {
    ...
    if (!IsMasked((node->node_info->attributes + i)->name,actual_mask))
    {
      ...
      /* Typ 'Bool': */
      if ((node->node_info->attributes + i)->type[0] == 'B' 
          && m3 == 0)
        m3 = 1;
      ...
      if ((((node->node_info->attributes + i)->type[0] == 'M')
          ...
          && ((node->node_info->attributes + i)->attr_value == NULL && m3 < 4))
        m3 = 4;
         ...
         || ((node->node_info->attributes + j)->type[0] == 'P'))
          {
           ...
           if ((node->node_info->attributes + j)->attr_value != NULL)
             {
...
/* Erzeugung und Plazierung der Such-Buttons: */
      ...
      if ((((node->node_info->attributes + j)->type[0] == 'M')
           || ((node->node_info->attributes + j)->type[0] == 'T')
           || ((node->node_info->attributes + j)->type[0] == 'P')) 
           && ((node->node_info->attributes + j)->attr_value != NULL))
      {
...

{
  ...
  attribute_list = Bim_ListOfAttributes();
...
{
  Cardinal n;
  Widget wl[XtNumber(nodem)],ll[XtNumber(nodem)];
